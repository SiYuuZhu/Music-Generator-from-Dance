...
def run(config_map):
  ...
  if FLAGS.mode == 'interpolate':
    ...
    results = model.sample(
        ...
        # length=config.hparams.max_seq_len,
        length=512,  # add my Vincent
        temperature=FLAGS.temperature)  # add my Vincent

  # filename = '%s_%s_%s-of-%03d.mid' % \
            #  (FLAGS.config, FLAGS.mode, date_and_time, FLAGS.num_outputs)
  filename = str(FLAGS.music_id) + '.mid'
  basename = os.path.join(
      FLAGS.output_dir,
      filename)
      # '%s_%s_%s-*-of-%03d.mid' %
...
def set_flags():
  ...
  FLAGS.checkpoint_file = './music_vae/checkpoints/hierdec-trio_16bar.tar'
  FLAGS.mode = 'interpolate'
  FLAGS.num_outputs = 1
  FLAGS.output_dir = './music_vae/generate/interpolate'
...
def main():
  ...
  set_flags()
  return run(configs.CONFIG_MAP)


def console_entry_point(input_midi_1, input_midi_2, music_id):
  tf.disable_v2_behavior()
  FLAGS.input_midi_1 = input_midi_1
  FLAGS.input_midi_2 = input_midi_2
  FLAGS.music_id = music_id
  # tf.app.run(main)
  return main()
